...
func insertDB(name string, is_followed bool, avatar_url_template string, url string, user_type string, answer_count int, url_token string, is_advertiser bool, avatar_url string, is_following bool, is_org bool, headline string, follower_count int, the_type string, id string, articles_count int) {
	...
	stmt, err := db.Prepare("INSERT " + table_name + " SET name=?,is_followed=?,avatar_url_template=?,url=?,user_type=?,answer_count=?,url_token=?,is_advertiser=?,avatar_url=?,is_following=?,is_org=?,headline=?,follower_count=?,the_type=?,id=?,articles_count=?")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func main() {
	...
	for url := range urlChannel {
		// 获取当前运行时的一些相关参数等
		fmt.Println("routines num = ", runtime.NumGoroutine(), "chan len = ", len(urlChannel), "From-URL = ", url)
